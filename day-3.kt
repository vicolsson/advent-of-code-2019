...
fun solvePuzzle(): String {
  ...
  sortedIntersections.sortBy { it -> abs(it.first) + abs(it.second) }

  val closest = sortedIntersections[1]
  return (abs(closest.first) + abs(closest.second)).toString()
}

fun getIntersections(firstPathPositions: List<Pair<Int, Int>>,
 secondPathPositions: List<Pair<Int, Int>>): List<Pair<Int, Int>> {
  return firstPathPositions.filter {
    it -> pathContainsPosition(secondPathPositions, it.first, it.second)
  }
}

fun pathContainsPosition(pathPositions: List<Pair<Int, Int>>, x: Int, y: Int): Boolean {
  return pathPositions.any { it -> it.first == x && it.second == y }
}

fun getPathPositions(path: List<String>): List<Pair<Int, Int>> {
  val pathPositions = mutableListOf<Pair<Int, Int>>()
  ...
  path.forEach {
    ...
    for (i in 0..distance) {
      when (direction) {
        'R' -> pathPositions.add(Pair(x + i, y))
        'L' -> pathPositions.add(Pair(x - i, y))
        'U' -> pathPositions.add(Pair(x, y - i))
        'D' -> pathPositions.add(Pair(x, y + i))
      }
    }
    when (direction) {
      'R' -> x += distance
