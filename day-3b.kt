fun solvePuzzle(): String {
  ...
  val firstPathPositions = getPathPositions(paths[0])
  val secondPathPositions = getPathPositions(paths[1])

  val intersections = getIntersections(firstPathPositions, secondPathPositions)
  
  val sortedIntersections = intersections.toMutableList()
  sortedIntersections.sortBy { it -> it.third }

  val shortest = sortedIntersections[1]
  return shortest.third.toString()
}

fun getIntersections(firstPathPositions: List<Triple<Int, Int, Int>>,
 secondPathPositions: List<Triple<Int, Int, Int>>): List<Triple<Int, Int, Int>> {
  return firstPathPositions
    .filter { it -> pathContainsPosition(secondPathPositions, it.first, it.second) > -1 }
    .map {
      it -> 
        val pathDistance = pathContainsPosition(secondPathPositions, it.first, it.second)
        Triple(it.first, it.second, it.third + pathDistance)
    }
}

fun pathContainsPosition(pathPositions: List<Triple<Int, Int, Int>>, x: Int, y: Int): Int {
  
  val intersections = pathPositions.filter { it -> it.first == x && it.second == y }.toMutableList()
  if (intersections.size == 0) {
    return -1
  }

  intersections.sortBy { it -> it.third }

  return intersections.first().third
}

fun getPathPositions(path: List<String>): List<Triple<Int, Int, Int>> {
  val pathPositions = mutableListOf<Triple<Int, Int, Int>>()
  ...
  path.forEach {
    ...
    val distance = it.removeRange(0, 1).toInt()

    for (i in 0..distance) {
      when (direction) {
        'R' -> pathPositions.add(Triple(x + i, y, pathDistance + i))
        'L' -> pathPositions.add(Triple(x - i, y, pathDistance + i))
        'U' -> pathPositions.add(Triple(x, y - i, pathDistance + i))
        'D' -> pathPositions.add(Triple(x, y + i, pathDistance + i))
      }
    }
    when (direction) {
      'R' -> x += distance
